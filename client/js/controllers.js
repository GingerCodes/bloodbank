app.controller("HomeController", function ($rootScope, $scope, Profile, Request, User) {
    ...
    $scope.search_doner = function () {
        if ($scope.search_form.$valid && typeof $scope.search.location_details != 'undefined') {
            var upper_lat = $scope.search.location_details.geometry.location.A + 0.03;
            var lower_lat = $scope.search.location_details.geometry.location.A - 0.03;
            var upper_lng = $scope.search.location_details.geometry.location.F + 0.03;
            var lower_lng = $scope.search.location_details.geometry.location.F - 0.03;

            $scope.match_donors = User.count({where: {"blood_group": $scope.search.blood_group,
...
app.controller("UserController", function ($rootScope, $scope, $location, User) {
    $scope.blood_groups = [
        'A+',
        'A-',
        'B+',
...
//            user.gender = true;
            user.$create({}, function (data) {
                Materialize.toast("Registration Completed Succesfully.");
                $location.path('/');
            }, function (err) {
                var messages = err.data.error.details.messages;
                for (var key in messages) {
                    if (messages.hasOwnProperty(key)) {
                        Materialize.toast(key + " " + messages[key]);
                    }
                }
            });
        } else {
            Materialize.toast("Please fill all the fields.");
        }
    };
    ...
    $scope.login = function () {
        if ($scope.loginForm.$valid) {
            User.login($scope.user).$promise.then(function (response) {
                ...
                Materialize.toast("Succesfully Logged In");
                $location.path('/');
            })
        } else {
